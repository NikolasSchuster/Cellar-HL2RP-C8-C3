...
plugin:Hook("CheckPassword", "serverguard.familysharing.CheckFamilySharing", function(communityID, ip, serverPassword, enteredPassword, name)
	...
	local url = string.format("http://api.steampowered.com/IPlayerService/IsPlayingSharedGame/v0001/?key=%s&format=json&steamid=%s&appid_playing=4000", "244CCC32B69273E8F119335CF5E1ABB7", communityID);
	...
	http.Fetch(url, function(body)
		...
		if (!response or !response.lender_steamid) then
			serverguard.PrintConsole("[familysharing] Failed to check for [" .. steamID .. "], JSON response not valid.\n");
			return;
		end;
		...
		if (response.lender_steamid != "0") then
			...
			if (lenderData) then
				...
				if (endTime == 0 or endTime >= os.time()) then
					serverguard.PrintConsole("[familysharing] Player [" .. steamID .. "] had family sharing set up with banned player '" .. tostring(lenderData.player) .. "' [" .. lenderSteamID .. "] and has been banned.\n");
