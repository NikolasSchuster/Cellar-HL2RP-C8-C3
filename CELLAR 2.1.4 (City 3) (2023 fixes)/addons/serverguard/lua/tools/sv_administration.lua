...
function serverguard:BanPlayer(admin, player, length, reason, bKick, bSilent, adminNameOverride)
	...
	elseif (type(player) == "string") then
		...
		if (IsValid(playerObj)) then
			...
			if string.find(player, "ANON") then
				player = AnonIDToSteamID(player)
			end
...
local function serverGuard_AddMenuBan(pPlayer, command, arguments)
	if (serverguard.player:HasPermission(pPlayer, "Ban")) then
		...
		if (IsValid(found)) then
			...
			local anonID = GenerateAnonID(steamID)
			
			if (length > 0) then
				local hours = math.Round(length / 3600);

				if (hours >= 1) then
					serverguard.Notify(nil, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(pPlayer), SERVERGUARD.NOTIFY.WHITE, " has banned ", SERVERGUARD.NOTIFY.RED, anonID, SERVERGUARD.NOTIFY.WHITE, " for ", SERVERGUARD.NOTIFY.RED, tostring(hours), SERVERGUARD.NOTIFY.WHITE, " hour(s). Reason: " .. reason);
				else
					serverguard.Notify(nil, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(pPlayer), SERVERGUARD.NOTIFY.WHITE, " has banned ", SERVERGUARD.NOTIFY.RED, anonID, SERVERGUARD.NOTIFY.WHITE, " for ", SERVERGUARD.NOTIFY.RED, tostring(math.Round(length / 60)), SERVERGUARD.NOTIFY.WHITE, " minute(s). Reason: " .. reason);
				end;
			else
				serverguard.Notify(nil, SERVERGUARD.NOTIFY.GREEN, serverguard.player:GetName(pPlayer), SERVERGUARD.NOTIFY.WHITE, " has banned ", SERVERGUARD.NOTIFY.RED, anonID, SERVERGUARD.NOTIFY.WHITE, ",", SERVERGUARD.NOTIFY.RED, " permanently", SERVERGUARD.NOTIFY.WHITE, ". Reason: " .. reason);
			end;
		end;
