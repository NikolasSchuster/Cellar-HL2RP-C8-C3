...
function PLUGIN:PlayerModelChanged(client, model, oldModel)
    if client:Team() == FACTION_VORTIGAUNT then
        if (model:lower() == "models/cellar/earl_roleplay/joe.mdl") or (model:lower() != "models/vortigaunt_slave.mdl") then
...
function PLUGIN:PostPlayerLoadout(client)
    if client:Team() == FACTION_VORTIGAUNT then
        if (model:lower() == "models/cellar/earl_roleplay/joe.mdl") or (client:GetModel():lower() != "models/vortigaunt_slave.mdl") then
            if !client:HasWeapon("ix_vortbeam") then
                client:Give("ix_vortbeam")
            end
